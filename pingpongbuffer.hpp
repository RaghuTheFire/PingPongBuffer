 ...
 */
uint8_t *ppbuf_dma_get_active_addr(ppbuf_t* p, int *size);

/**
 * @brief USE WITH DMA ONLY! get the current inactive buffer address
 */
uint8_t *ppbuf_dma_get_inactive_addr(ppbuf_t* p, int *size);
...
#define PPBUF_DECLARE(name,size)                               \
    std::vector<uint8_t> ppbuf_mem_##name(size * 2, 0);      \
    ppbuf_t name = 
    {                                         \
        .buffer_data = ppbuf_mem_##name,                    \
        .ping = 1,                                           \
        .pong = 0,                                           \
        .buffer_size = size,                                 \
        .put_index = 0,                                     \
